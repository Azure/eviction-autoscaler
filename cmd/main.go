...
func main() {
	...
	// Parse PDB_MAX_UNAVAILABLE_ADOPTION environment variable (defaults to false).
	// When enabled, user-authored PDBs that use maxUnavailable are converted once to
	// an absolute minAvailable policy and managed like autogenerated PDBs.
	pdbMaxUnavailableAdoptionStr := os.Getenv("PDB_MAX_UNAVAILABLE_ADOPTION")
	pdbMaxUnavailableAdoption := false
	if pdbMaxUnavailableAdoptionStr != "" {
		var err error
		pdbMaxUnavailableAdoption, err = strconv.ParseBool(pdbMaxUnavailableAdoptionStr)
		if err != nil {
			setupLog.Error(err, "Failed to parse PDB_MAX_UNAVAILABLE_ADOPTION env variable")
			os.Exit(1)
		}
	}
	setupLog.Info("PDB maxUnavailable adoption configuration",
		"pdbMaxUnavailableAdoption", pdbMaxUnavailableAdoption)
	...
	if pdbCreate || pdbMaxUnavailableAdoption {
		if err = (&controllers.DeploymentToPDBReconciler{
			Client:             mgr.GetClient(),
			Scheme:             mgr.GetScheme(),
			Filter:             nsfilter,
			PDBCreationEnabled: pdbCreate,
	...
	if err = (&controllers.PDBToEvictionAutoScalerReconciler{
		...
		Scheme:              mgr.GetScheme(),
		Filter:              nsfilter,
		AdoptMaxUnavailable: pdbMaxUnavailableAdoption,
	}).SetupWithManager(mgr); err != nil {
