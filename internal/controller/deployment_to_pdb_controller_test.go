...
var _ = Describe("DeploymentToPDBReconciler ownership transfer", func() {
	var (
		namespace  string
		deployment *appsv1.Deployment
		r          *DeploymentToPDBReconciler
	...
	const deploymentName = "ownership-test-deployment"
	...
	It("should remove owner reference when annotation is removed", func() {
		...
		// Verify PDB was created with owner reference
		pdb := &policyv1.PodDisruptionBudget{}
		err = k8sClient.Get(ctx, client.ObjectKey{Namespace: namespace, Name: deploymentName}, pdb)
		Expect(err).ToNot(HaveOccurred())
		...
		// Reconcile again - should remove owner reference
		_, err = r.Reconcile(ctx, req)
		Expect(err).ToNot(HaveOccurred())
	...
	It("should add owner reference back when annotation is re-added", func() {
		...
		// Reconcile - should remove owner reference
		_, err = r.Reconcile(ctx, req)
		Expect(err).ToNot(HaveOccurred())
		...
		// Reconcile again - should add owner reference back
		_, err = r.Reconcile(ctx, req)
		Expect(err).ToNot(HaveOccurred())
