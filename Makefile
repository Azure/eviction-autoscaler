...
RELEASEIMG ?= docker.io/paulgmiller/eviction-autoscaler:$(TAG)

.PHONY: release
release: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image paulgmiller/k8s-pdb-autoscaler=$(RELEASEIMG)
	$(KUSTOMIZE) build config/default > releases/$(TAG).yaml
