...
const OriginalMaxUnavailableAnnotationKey = "eviction-autoscaler.azure.com/original-max-unavailable"

// originalMaxUnavailable returns the relative disruption allowance preserved when
// a user-authored PDB is adopted. The JSON encoding preserves whether the user set
// an integer or a percentage.
func originalMaxUnavailable(pdb *policyv1.PodDisruptionBudget) (*intstr.IntOrString, bool, error) {
	if pdb == nil || pdb.Annotations == nil {
		return nil, false, nil
	}
	raw, ok := pdb.Annotations[OriginalMaxUnavailableAnnotationKey]
	if !ok {
		return nil, false, nil
	}
...
// equivalent absolute healthy-pod floor at replicas. Kubernetes rounds percentage
// maxUnavailable values up, so the same rounding is used here.
func minAvailableForMaxUnavailable(maxUnavailable *intstr.IntOrString, replicas int32) (int32, error) {
	resolved, err := intstr.GetScaledValueFromIntOrPercent(maxUnavailable, int(replicas), true)
	if err != nil {
		return 0, fmt.Errorf("resolving maxUnavailable: %w", err)
	}
	if resolved < 0 {
		return 0, fmt.Errorf("resolving maxUnavailable: value must not be negative")
	}
	minAvailable := int(replicas) - resolved
	if minAvailable < 0 {
		minAvailable = 0
	}
...
// managedMinAvailable computes the absolute floor maintained by the Deployment and
// autoscaler PDB controllers. Autogenerated PDBs allow zero disruptions; adopted PDBs
// retain the user's preserved maxUnavailable allowance.
func managedMinAvailable(pdb *policyv1.PodDisruptionBudget, replicas int32) (int32, error) {
...
// by comparing managed annotations and disruption fields between old and new PDB.
func triggerOnPDBAnnotationChange(e event.UpdateEvent, logger logr.Logger) bool {
	...
	if okOld && okNew {
		...
		oldMax := tryGet(oldPDB.Annotations, OriginalMaxUnavailableAnnotationKey)
		newMax := tryGet(newPDB.Annotations, OriginalMaxUnavailableAnnotationKey)
		if oldMax != newMax {
			logger.Info("PDB update event detected, preserved maxUnavailable changed",
				"namespace", newPDB.Namespace, "name", newPDB.Name)
			return true
		}
